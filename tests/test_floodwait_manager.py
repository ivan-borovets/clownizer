...
@pytest.mark.asyncio
async def test_floodwait_manager_handle(
    flood_wait_error: FloodWait, test_client_for_custom_client: CustomClient
) -> None:
    client: CustomClient = test_client_for_custom_client
    ...
    assert (
        f"FloodWait is provoked...|{flood_wait_error.value} s to wait" in logged_message
    )
